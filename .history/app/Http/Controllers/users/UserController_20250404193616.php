...
class UserController extends Controller
{
    ...
    public function update(Request $request, User $user)
    {
        ...
        if($request->hasFile('photo')){ //Compruebo que me llega la foto nueva de usuario
            if($user->photo){//Compruebo que el usuario tenga una foto de perfil antigua a borrar
                ...
                $oldPhoto = public_path('profile/'.$user->photo);
            ...
            $profilePhoto = $imgManager->read(public_path("profile/").$filename);

            $profilePhoto->resize(300,300);

            $profilePhoto->save(public_path("profile/").$filename);
    ...
    public function destroy(User $user)
    {
        $user->deleted = true;
        $user->save();
        return redirect()->route('users.index', ['Success' => 'Usuario desactivado con suceso']);
    } 
}
