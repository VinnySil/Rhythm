<x-app-layout>
    <div class="sm:w-2/4 w-11/12 py-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg mt-5 m-auto text-center max-w-2xl">
        <h1 class="dark:text-white text-2xl p-4">Modificar usuario</h1>
        <form method="POST" enctype="multipart/form-data" action={{ route('users.update', $user)}} method="POST" id="edit-form" class="md:py-4 md:w-2/3 m-auto">
            @csrf
            @method('PATCH')  
            <div id="main-container" class="md:flex grid gap-10">
                <div id="photo-container" class="md:border-r-2 md:border-b-0 border-b-2 border-pink-400 flex justify-center p-4">
                    <img src={{asset('build/'.$user->photo)}} alt={{$user->photo}} class="w-40 md:w-40 md:h-40">   
                </div>
